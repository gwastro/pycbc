...
from pycbc.workflow.core import Executable, File, FileList, makedir
...
def make_template_plot(workflow, bank_file, out_dir, bins=None, tags=None):
    tags = [] if tags is None else tags
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "plot_bank", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
    node.add_input_opt("--bank-file", bank_file)

    if workflow.cp.has_option_tags(
        "workflow-coincidence", "background-bins", tags=tags
    ):
        bins = workflow.cp.get_opt_tags(
            "workflow-coincidence", "background-bins", tags=tags
        )
    if bins:
...
def make_range_plot(
    workflow, psd_files, out_dir, exclude=None, require=None, tags=None
):
    tags = [] if tags is None else tags
    makedir(out_dir)
...
def make_spectrum_plot(
    workflow, psd_files, out_dir, tags=None, hdf_group=None, precalc_psd_files=None
):
    tags = [] if tags is None else tags
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "plot_spectrum", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
    node.add_input_list_opt("--psd-files", psd_files)
    node.new_output_file_opt(workflow.analysis_time, ".png", "--output-file")
...
def make_segments_plot(workflow, seg_files, out_dir, tags=None):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "plot_segments", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
...
def make_gating_plot(workflow, insp_files, out_dir, tags=None):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "plot_gating", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
...
def make_throughput_plot(workflow, insp_files, out_dir, tags=None):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "plot_throughput", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
    node.add_input_list_opt("--input-file", insp_files)
    node.new_output_file_opt(workflow.analysis_time, ".png", "--output-file")
    workflow += node
...
def make_foreground_table(
    ...
    if hierarchical_level is not None and tags:
        tags = [(f"HIERARCHICAL_LEVEL_{hierarchical_level:02d}")] + tags
    elif hierarchical_level is not None and not tags:
        tags = [f"HIERARCHICAL_LEVEL_{hierarchical_level:02d}"]
...
def make_sensitivity_plot(
    workflow, inj_file, out_dir, exclude=None, require=None, tags=None
):
    tags = [] if tags is None else tags
    makedir(out_dir)
...
def make_inj_table(workflow, inj_file, out_dir, missed=False, singles=None, tags=None):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "page_injections", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
...
def make_seg_table(
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "page_segtable", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
    node.add_input_list_opt("--segment-files", seg_files)
    quoted_seg_names = []
    for s in seg_names:
...
def make_veto_table(workflow, out_dir, vetodef_file=None, tags=None):
    ...
    if vetodef_file is None:
        if not workflow.cp.has_option_tags(
            "workflow-segments", "segments-veto-definer-file", []
        ):
            return None
        vetodef_file = workflow.cp.get_opt_tags(
            "workflow-segments", "segments-veto-definer-file", []
        )
        file_url = urljoin("file:", pathname2url(vetodef_file))
        vdf_file = File(
            workflow.ifos, "VETO_DEFINER", workflow.analysis_time, file_url=file_url
        )
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "page_vetotable", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
...
def make_seg_plot(workflow, seg_files, out_dir, seg_names=None, tags=None):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "page_segplot", ifos=workflow.ifos, out_dir=out_dir, tags=tags
    ).create_node()
    node.add_input_list_opt("--segment-files", seg_files)
    quoted_seg_names = []
    for s in seg_names:
...
def make_ifar_plot(
    ...
    makedir(out_dir)
    exe = PlotExecutable(
        workflow.cp, executable, ifos=trigger_file.ifo_list, out_dir=out_dir, tags=tags
    )
    node = exe.create_node()
...
def make_farstat_plot(
    ...
    exe = PlotExecutable(
        workflow.cp, executable, ifos=workflow.ifos, out_dir=out_dir, tags=tags
    )
    node = exe.create_node()
...
def make_snrratehist_plot(
    workflow, bg_file, out_dir, closed_box=False, tags=None, hierarchical_level=None
):
...
def make_snrifar_plot(
    ...
    makedir(out_dir)
    exe = PlotExecutable(
        workflow.cp, "plot_snrifar", ifos=bg_file.ifo_list, out_dir=out_dir, tags=tags
    )
    node = exe.create_node()
    node.add_input_opt("--trigger-file", bg_file)
    if hierarchical_level is not None:
...
def make_results_web_page(
    workflow, results_dir, template="orange", explicit_dependencies=None
):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "results_page", ifos=workflow.ifos, out_dir=out_dir
    ).create_node()
...
def make_dq_segment_table(workflow, dq_file, out_dir, tags=None):
    ...
    makedir(out_dir)
    node = PlotExecutable(
        workflow.cp, "page_dq_table", ifos=dq_file.ifo, out_dir=out_dir, tags=tags
    ).create_node()
    node.add_input_opt("--dq-file", dq_file)
    node.add_opt("--ifo", dq_file.ifo)
...
def make_bank_compression_plots(workflow, bank_files, out_dir, tags=None):
    ...
    for tag in secs:
        ...
        node.add_input_list_opt("--bank-files", bank_files)

        node.new_output_file_opt(workflow.analysis_time, ".png", "--output")
        workflow += node
        files += node.output_files
    return files
