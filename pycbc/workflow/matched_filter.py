...
def setup_matchedfltr_workflow(
    ...
    mfltrMethod = cp.get_opt_tags(
        "workflow-matchedfilter", "matchedfilter-method", tags
    )
...
def setup_matchedfltr_dax_generated(
    workflow,
    science_segs,
    datafind_outs,
    tmplt_banks,
...
def setup_matchedfltr_dax_generated_multi(
    ...
    if match_fltr_exe == "pycbc_multi_inspiral":
        ...
        n_sg = sum(bool_sg)
        if n_sg == 0:
            cp.set("inspiral", "ra", cp.get("workflow", "ra"))
            cp.set("inspiral", "dec", cp.get("workflow", "dec"))
        elif n_sg > 1:
        ...
        if cp.has_option("workflow", "do-long-slides") and "slide" in tags[-1]:
            ...
            logger.info("Setting up matched-filtering for slide %d", slide_num)
            slide_shift = int(cp.get("inspiral", "segment-length"))
            time_slide_dict = {
                ifo: (slide_num + 1) * ix * slide_shift for ix, ifo in enumerate(ifos)
            }
            multi_ifo_coherent_job_setup(
                workflow,
                inspiral_outs,
