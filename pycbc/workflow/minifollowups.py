...
def setup_foreground_minifollowups(
    ...
    node.add_multiifo_input_list_opt(
        "--single-detector-triggers", single_triggers
    )
    ...
    node.new_output_file_opt(
        workflow.analysis_time, ".dax", "--output-file", tags=tags
    )
    node.new_output_file_opt(
        workflow.analysis_time, ".dax.map", "--output-map", tags=tags
    )
    node.new_output_file_opt(
        workflow.analysis_time, ".tc.txt", "--transformation-catalog", tags=tags
    )
    ...
    try:
        staging_site = workflow.cp.get(
            "workflow-foreground_minifollowups", "staging-site"
        )
    except:
        staging_site = None
    ...
    Workflow.set_job_properties(
        job, map_file, tc_file, staging_site=staging_site
    )
...
def setup_single_det_minifollowups(
    ...
    try:
        staging_site = workflow.cp.get(
            "workflow-sngl_minifollowups", "staging-site"
        )
    except:
        staging_site = None
...
def setup_injection_minifollowups(
    ...
    try:
        staging_site = workflow.cp.get(
            "workflow-injection_minifollowups", "staging-site"
        )
    except:
        staging_site = None
    ...
    for tag in secs:
        for ifo in workflow.ifos:
            ...
            if use_exact_inj_params:
                ...
                node.add_opt(
                    "--template-start-frequency", "%.6f" % params["f_lower"]
                )
                # Is this precessing?
                if "u_vals" in params or "u_vals_%s" % ifo in params:
                    ...
                    node.add_opt(
                        "--inclination", "%.6f" % params["inclination"]
                    )
                    try:
                        node.add_opt("--u-val", "%.6f" % params["u_vals"])
                    except:
                        node.add_opt(
                            "--u-val", "%.6f" % params["u_vals_%s" % ifo]
                        )
...
def make_inj_info(
    workflow, injection_file, injection_index, num, out_dir, tags=None
):
...
def make_sngl_ifo(
    workflow, sngl_file, bank_file, trigger_id, out_dir, ifo, tags=None
):
