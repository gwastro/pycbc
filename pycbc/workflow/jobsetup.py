    ...
    * END LOOPING OVER SCIENCE SEGMENTS
    ...
    for curr_seg in science_segs:
        ...
        segmenter = JobSegmenter(
            data_length, valid_chunk, valid_length, curr_seg, curr_exe_job
        )

        for job_num in range(segmenter.num_jobs):
            ############## (3) #############
            # Figure out over what times this job will be valid for
            job_valid_seg = segmenter.get_valid_times_for_job(
                job_num, allow_overlap=allow_overlap
            )
            ...
            if parents:
                ...
                curr_parent = parents.find_outputs_in_range(
                    ifo, job_valid_seg, useSplitLists=True
                )
                if not curr_parent:
            ...
            curr_dfouts = None
            if datafind_outs:
                curr_dfouts = datafind_outs.find_all_output_in_range(
                    ifo, job_data_seg, useSplitLists=True
                )
                if not curr_dfouts:
            ...
            for parent in curr_parent:
                ...
                node = curr_exe_job.create_node(
                    job_data_seg, job_valid_seg, parent=parent, df_parents=curr_dfouts
                )
                ...
                curr_out_files = [i for i in curr_out_files if "PSD_FILE" not in i.tags]
                out_files += curr_out_files

    return out_files


def multi_ifo_coherent_job_setup(
    workflow,
    out_files,
    curr_exe_job,
    science_segs,
    ...
    if curr_exe_job.injection_file is None:
        for split_bank in parents:
            ...
            node = curr_exe_job.create_node(
                ...
                job_valid_seg,
                parent=split_bank,
                dfParents=input_files,
                bankVetoBank=bank_veto,
                skygrid_file=skygrid_file,
...
class JobSegmenter:
    ...
    def __init__(
        self, data_lengths, valid_chunks, valid_lengths, curr_seg, curr_exe_class
    ):
        """Initialize class."""
        ...
        self.data_length, self.valid_chunk, self.valid_length = self.pick_tile_size(
            self.curr_seg_length, data_lengths, valid_chunks, valid_lengths
        )
        ...
        self.num_jobs = int(
            math.ceil(
                (self.curr_seg_length - self.data_loss) / float(self.valid_length)
            )
        )
        ...
        if self.curr_seg_length == self.data_length:
            ...
            self.job_time_shift = (self.curr_seg_length - self.data_length) / float(
                self.num_jobs - 1
            )
    ...
    def get_valid_times_for_job(self, num_job, allow_overlap=True):
        ...
        if not allow_overlap:
            data_per_job = (self.curr_seg_length - self.data_loss) / float(
                self.num_jobs
            )
            lower_boundary = (
                num_job * data_per_job + self.valid_chunk[0] + self.curr_seg[0]
            )
            ...
            if lower_boundary < job_valid_seg[0] or upper_boundary > job_valid_seg[1]:
                err_msg = (
                    "Workflow is attempting to generate output "
                    "from a job at times where it is not valid."
                )
                raise ValueError(err_msg)
            job_valid_seg = segments.segment([lower_boundary, upper_boundary])
    ...
    def get_data_times_for_job(self, num_job):
        ...
        if hasattr(self.exe_class, "zero_pad_data_extend"):
            job_data_seg = self.exe_class.zero_pad_data_extend(
                job_data_seg, self.curr_seg
            )
...
class PyCBCInspiralExecutable(Executable):
    ...
    def create_node(self, data_seg, valid_seg, parent=None, df_parents=None, tags=None):
        ...
        if not self.has_opt("pad-data"):
            raise ValueError(
                "The option pad-data is a required option of "
        ...
        node.add_profile(
            "pegasus", "relative.submit.dir", self.pegasus_name + "_" + curr_rel_dir
        )
    ...
    def get_valid_times(self):
        ...
        if self.cp.has_option("workflow-matchedfilter", "min-analysis-segments"):
            min_analysis_segs = int(
                self.cp.get("workflow-matchedfilter", "min-analysis-segments")
            )
        else:
            min_analysis_segs = 0

        if self.cp.has_option("workflow-matchedfilter", "max-analysis-segments"):
            max_analysis_segs = int(
                self.cp.get("workflow-matchedfilter", "max-analysis-segments")
            )
        else:
        ...
        if self.cp.has_option("workflow-matchedfilter", "min-analysis-length"):
            min_analysis_length = int(
                self.cp.get("workflow-matchedfilter", "min-analysis-length")
            )
        else:
        ...
        if self.cp.has_option("workflow-matchedfilter", "max-analysis-length"):
            max_analysis_length = int(
                self.cp.get("workflow-matchedfilter", "max-analysis-length")
            )
        else:
...
class PyCBCMultiInspiralExecutable(Executable):
    ...
    def __init__(
        ...
        self.injection_file = injection_file
        self.data_seg = segments.segment(
            int(cp.get("workflow", "start-time")), int(cp.get("workflow", "end-time"))
        )
        self.num_threads = 1
    ...
    def create_node(
        ...
        if not dfParents:
            raise ValueError(
                "%s must be supplied with frame or cache files" % self.name
            )

        # If doing single IFO search, make sure slides are disabled
        if len(self.ifo_list) < 2 and (
            self.get_opt("--do-short-slides") is not None
            or self.get_opt("--short-slide-offset") is not None
        ):
            raise ValueError(
                "Cannot run with time slides in a single IFO "
        ...
        node.new_output_file_opt(
            data_seg, ".hdf", "--output", tags=tags, store_file=self.retain_files
        )
        node.add_input_opt(
            "--bank-file",
            parent,
        ...
        if inj_file is not None:
            if (
                ...
                raise ValueError(
                    "Cannot run with short slides in an "
        ...
        channel_names_str = " ".join([val for key, val in channel_names.items()])
    ...
    def get_valid_times(self):
        ...
        if self.has_opt("segment-start-pad"):
            ...
            valid_start = self.data_seg[0] + deadtime - spec_len + pad_data - safety
...
class PyCBCTmpltbankExecutable(Executable):
    ...
    def create_node(self, data_seg, valid_seg, parent=None, df_parents=None, tags=None):
        ...
        if not df_parents:
            raise ValueError("%s must be supplied with data file(s)" % self.name)
    ...
    def create_nodata_node(self, valid_seg, tags=None):
        ...
        node.new_output_file_opt(
            valid_seg, ".xml.gz", "--output-file", store_file=self.retain_files
        )
...
class LigolwAddExecutable(Executable):
    ...
    def create_node(
        self, jobSegment, input_files, output=None, use_tmp_subdirs=True, tags=None
    ):
        if tags is None:
            tags = []
        node = Node(self)
...
class LalappsInspinjExecutable(Executable):
    ...
    def create_node(self, segment, exttrig_file=None, tags=None):
        ...
        if (
            self.get_opt("l-distr") == "exttrig"
            and exttrig_file is not None
        ...
        elif (self.get_opt("l-distr") != "exttrig") and (
            self.get_opt("l-distr") != "ipn" and not self.has_opt("ipn-file")
        ):
        ...
        if triggered:
            num_injs = int(
                self.cp.get_opt_tags("workflow-injections", "num-injs", curr_tags)
            )
        ...
        node.new_output_file_opt(
            segment, self.extension, "--output", store_file=self.retain_files
        )
...
class PycbcSplitBankExecutable(Executable):
    ...
    def __init__(self, cp, exe_name, num_banks, ifo=None, out_dir=None):
        super().__init__(cp, exe_name, ifo, out_dir, tags=[])
...
class PycbcCreateInjectionsExecutable(Executable):
    ...
    def create_node(self, config_files=None, seed=None, tags=None):
        ...
        injection_file = node.new_output_file_opt(
            analysis_time, self.extension, "--output-file", tags=tags
        )
...
class PycbcInferenceExecutable(Executable):
    ...
    def create_node(self, config_file, seed=None, tags=None, analysis_time=None):
        """
        ...
        if seed is not None:
            node.add_opt("--seed", seed)
        inference_file = node.new_output_file_opt(
            analysis_time, ".hdf", "--output-file", tags=tags
        )
        if self.cp.has_option("pegasus_profile-inference", "condor|+CheckpointSig"):
