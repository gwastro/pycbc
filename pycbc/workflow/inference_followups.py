...
def make_inference_plot(
    ...
    node = PlotExecutable(
        workflow.cp, name, ifos=workflow.ifos, out_dir=output_dir, tags=tags
    ).create_node()
    ...
    if isinstance(input_file, list):
        ...
        node.add_input_list_opt(f"--{input_file_opt}", input_file)
    else:
        # assume just a single file
        node.add_input_opt(f"--{input_file_opt}", input_file)
    node.new_output_file_opt(analysis_seg, output_file_extension, "--output-file")
...
def make_inference_prior_plot(
    workflow, config_file, output_dir, name="plot_prior", analysis_seg=None, tags=None
):
    """
...
def create_posterior_files(
    workflow,
    samples_files,
    output_dir,
    parameters=None,
    name="extract_posterior",
    analysis_seg=None,
    tags=None,
):
    """
    ...
    if workflow.cp.has_option(name, "parameters"):
        parameters = workflow.cp.get(name, "parameters")
        workflow.cp.remove_option(name, "parameters")
    extract_posterior_exe = Executable(
        workflow.cp, name, ifos=workflow.ifos, out_dir=output_dir
    )
...
def make_inference_skymap(
    workflow, fits_file, output_dir, name="plot_skymap", analysis_seg=None, tags=None
):
    """
...
def make_inference_summary_table(
    workflow,
    inference_file,
    output_dir,
    parameters=None,
...
def make_inference_acceptance_rate_plot(
    workflow,
    inference_file,
    output_dir,
...
def make_inference_plot_mcmc_history(
    workflow,
    inference_file,
    output_dir,
...
def make_inference_dynesty_run_plot(
    workflow,
    inference_file,
    output_dir,
...
def make_inference_dynesty_trace_plot(
    workflow,
    inference_file,
    output_dir,
...
def make_inference_pp_table(
    workflow,
    posterior_files,
    output_dir,
    parameters=None,
    injection_samples_map=None,
    ...
    if injection_samples_map is not None:
        node.add_opt(
            "--injection-samples-map", _params_for_pegasus(injection_samples_map)
        )
    workflow += node
    return node.output_files


def make_inference_pp_plot(
    workflow,
    posterior_files,
    output_dir,
    parameters=None,
...
def make_inference_inj_recovery_plot(
    workflow,
    posterior_files,
    output_dir,
    parameter,
...
def get_plot_group(cp, section_tag):
    ...
    opts = [
        opt
        for opt in cp.options(f"workflow-{section_tag}")
...
def make_diagnostic_plots(workflow, diagnostics, samples_file, label, rdir, tags=None):
    """
    ...
    if "dynesty_traceplot" in diagnostics:
        ...
        base = f"dynesty_traceplot/{label}"
        dynesty_trace_plots = []
        for kk, sf in enumerate(samples_file):
...
def make_posterior_workflow(
    ...
    * **Posterior plots**: additional posterior plots are created using the
    ...
    table_params = workflow.cp.get_opt_tag("workflow", "table-params", "summary_table")
    # get any metadata that should be printed
    if workflow.cp.has_option("workflow-summary_table", "print-metadata"):
        table_metadata = workflow.cp.get_opt_tag(
            "workflow", "print-metadata", "summary_table"
        )
    else:
        table_metadata = None

    # figure out if we are making a skymap
    make_skymap = "create_fits_file" in workflow.cp.options(
        "executables"
