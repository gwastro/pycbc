...
try:
    ...
    for approx_enum in range(lalsimulation.NumApproximants):
...
def print_td_approximants():
    ...
    for approx in _lalsim_td_approximants:
        print("  " + approx)
    print("CUDA Approximants")
    for approx in _cuda_td_approximants:
        print("  " + approx)


def print_fd_approximants():
    print("LalSimulation Approximants")
    for approx in _lalsim_fd_approximants:
        print("  " + approx)
    print("CUDA Approximants")
    for approx in _cuda_fd_approximants:
        print("  " + approx)


def print_sgburst_approximants():
    print("LalSimulation Approximants")
    for approx in _lalsim_sgburst_approximants:
        print("  " + approx)
...
def get_waveform_filter(out, template=None, **kwargs):
    ...
    if input_params["approximant"] in td_approximants(_scheme.mgr.state):
        wav_gen = td_wav[type(_scheme.mgr.state)]
...
def td_waveform_to_fd_waveform(waveform, out=None, length=None, buffer_length=100):
    ...
    if len(waveform) > N:
        ...
        err_msg += f"Waveform length (in samples): {len(waveform)}"
        err_msg += f". Intended length: {N}."
        raise ValueError(err_msg)
