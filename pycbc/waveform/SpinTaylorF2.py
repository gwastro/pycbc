...
from math import acos, atan2, cos, log, pow, sin

import numpy
from numpy import complex128, double, sqrt
...
spintaylorf2_kernel = ElementwiseKernel(
    """pycuda::complex<double> *htildeP,
    ...
    "spintaylorf2_kernel",
    preamble=preamble,
    options=pkg_config_header_strings([]),
)
...
def spintaylorf2(**kwds):
    ...
    v0 = pow(piM * kmin * delta_f, 1.0 / 3)
    chi = sqrt(spin1x**2 + spin1y**2 + spin1z**2)
    kappa = (
        (lnhatx * spin1x + lnhaty * spin1y + lnhatz * spin1z) / chi
        if (chi > 0.0)
        else 1.0
    )
    ...
    ) * (mass1 * mass1 * chi * chi / M / M)
    pn_gamma = (
        (
            5.0 * (146597.0 + 7056.0 * eta) * mass1 / (2268.0 * M)
    ...
    dtdv5 = (-7729.0 / 672.0 + 13.0 * eta / 8.0) * PI + 9.0 * pn_gamma / 40.0

    #####Calculate the Initial Euler Angles alpha_ref, beta_ref=0 and zeta_ref#####
    gam = gamma0 * v0
    sqrtfac = sqrt(1.0 + 2.0 * kappa * gam + gam * gam)
    logv0 = log(v0)
    logfac1 = log(1.0 + kappa * gam + sqrtfac)
    ...
    alpha_ref = (
        ...
        * (
            ...
            * (
                ...
                + dtdv5 * kappa * v0 / (6.0 * gamma0)
                - dtdv5 * kappa2 / (2.0 * gamma02)
                - 1 / (3.0 * v03)
                - gamma0 * kappa / (6.0 * v02)
                - dtdv2 / v0
                - gamma02 / (3.0 * v0)
                + gamma02 * kappa2 / (2.0 * v0)
                + dtdv5 * v02 / 3.0
            )
        )
        - alpha0
    ...
    zeta_ref = prec_fac0 * (
        ...
        * (
        ...
        + logv0 * (kappa * gamma03 / 2.0 - gamma03 * kappa3 / 2.0)
        + logfac1
        * (
            dtdv2 * gamma0 * kappa
            + dtdv3
        ...
        - 1 / (3.0 * v03)
        - gamma0 * kappa / (2.0 * v02)
        - dtdv2 / v0
        + dtdv4 * gamma0 * kappa * v02 / 2.0
        + dtdv5 * v02 / 2.0
        + sqrtfac
        * (
            -dtdv3
        ...
        + dtdv5 * gamma0 * kappa * v03 / 3.0
    )

    #####Calculate the Complex sideband factors, mm=2 is first entry#####
    RE_SBfac0 = (1.0 + cos(thetaJ) ** 2) / 2.0
    RE_SBfac1 = sin(2.0 * thetaJ)
    RE_SBfac2 = 3.0 * sin(thetaJ) ** 2
    RE_SBfac3 = -sin(2.0 * thetaJ)
    RE_SBfac4 = (1.0 + cos(thetaJ) ** 2) / 2.0
    IM_SBfac0 = -cos(thetaJ)
    IM_SBfac1 = -2.0 * sin(thetaJ)
    IM_SBfac2 = 0.0
    IM_SBfac3 = -2.0 * sin(thetaJ)
    ...
    pfa6 = (
        (11583.231236531 / 4.694215680 - 640.0 / 3.0 * PI * PI - 6848.0 / 21.0 * GAMMA)
        + eta
        * (
            -15335.597827 / 3.048192
            + 2255.0 / 12.0 * PI * PI
            - 1760.0 / 3.0 * theta
            + 12320.0 / 9.0 * lambdaa
        )
    ...
    FTl6 = -8.56 / 1.05
    FTa7 = -(162.85 / 5.04 - 214.745 / 1.728 * eta - 193.385 / 3.024 * eta * eta) * PI

    dETaN = 2 * -eta / 2.0
    dETa1 = 2 * -(3.0 / 4.0 + 1.0 / 12.0 * eta)
    dETa2 = 3 * -(27.0 / 8.0 - 19.0 / 8.0 * eta + 1.0 / 24.0 * eta * eta)
    dETa3 = 4 * -(
        67.5 / 6.4
        - (344.45 / 5.76 - 20.5 / 9.6 * PI * PI) * eta
        + 15.5 / 9.6 * eta * eta
    ...
    spintaylorf2_kernel(
        htildeP.data[kmin:kmax],
        htildeC.data[kmin:kmax],
        kmin,
        phase_order,
        amplitude_order,
        delta_f,
