...
/* =====================================================================
 *
 * FAST CUBIC INTERPOLATOR (MAIN) - NEWLY ADDED
 *
 * =====================================================================
 */
                          ...
                          const int64_t imin)
{
    ...
    // sflen>=5 (>=4 segs) -> linear, quadratic, cubic..., quadratic, quadratic
    ...
    for (int64_t i = imin; i < sflen-1; i++) {
        ...
        // === Boundary 2: Last segment (i == sflen - 2) ===
        // Must use quadratic (points i-1, i, i+1)
        else if (i == sflen - 2 || sflen < 5) { // Also degrade if not enough points for cubic ever
            f0 = sample_frequencies[i-1];
            a0 = amp[i-1];
            p0 = phase[i-1];
            _decomp_qcode_segment<double, std::complex<double>>(
