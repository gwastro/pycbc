...
def default_modes(approximant):
    ...
    if approximant in ["IMRPhenomXPHM", "IMRPhenomXHM"]:
        ...
        raise ValueError(
            f"I don't know what the default modes are for approximant {approximant}, sorry!"
        )
    return ma
...
def get_glm(l, m, theta):
    r"""
    The maginitude of the :math:`{}_{-2}Y_{\ell m}`.
...
def get_fd_waveform_modes(template=None, **kwargs):
    ...
    if apprx not in _mode_waveform_fd:
        raise ValueError(f"I don't support approximant {apprx}, sorry")
