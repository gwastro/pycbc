...
def get_imrphenomxh_modes(return_posneg=False, **params):
    ...
    for (l, m) in mode_array:
        ...
        hclm =  0.5j * hlm # Cross strain, needs to be multiplied by SpherHarmonic. (-1)**(l) factor ALREADY included in LAL FDOneMode function
        if (m > 0):
            hclm *= -1
        hlms[l, m] = (hplm,hclm)
    return hlms
...
_mode_waveform_fd = {'IMRPhenomXHM': get_imrphenomhm_modes
                     #'IMRPhenomXPHM' : get_imrphenomhm_modes,          # IMRPhenomXPHM function must be implemented yet. 
                    }
