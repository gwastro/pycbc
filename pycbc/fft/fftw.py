...
def _fftw_plan_with_nthreads(nthreads):
    ...
    if not HAVE_FFTW_THREADED:
        if nthreads > 1:
            raise ValueError(
                f"Threading is NOT enabled, but {nthreads} > 1 threads specified"
            )
...
def _init_threads(backend):
    ...
    if _fftw_threaded_set:
        raise RuntimeError(
            f"Threading backend for FFTW already set to {_fftw_threaded_lib}; cannot be changed"
        )
    try:
        ...
        raise ValueError(
            f"Backend {backend} for FFTW threading does not exist!"
        )
...
def set_threads_backend(backend=None):
    ...
    if backend is not None:
        ...
        if retval != 0:
            raise RuntimeError(
                f"Could not initialize FFTW threading backend {backend}"
            )
    else:
...
def wisdom_io(filename, precision, action):
    ...
    if retval == 0:
        raise RuntimeError(
            f"Could not {action} wisdom from file {filename}"
        )
...
def verify_fft_options(opt, parser):
    ...
    if opt.fftw_measure_level not in [0, 1, 2, 3]:
        parser.error(
            f"{opt.fftw_measure_level} is not a valid FFTW measure level."
        )
