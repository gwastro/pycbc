...
class Tmpltbank:
    ...
    information, which will be used in later stages of the dag, these are:

    self.get_bank() -> The location of the template bank file, this may not
        exist yet if the bank will be generated by the workflow. In that case
        this gives the location that the bank will be written to.
        Will fail if the bank has not yet been set.
    self.set_bank(filePath) -> Sets the location of the template bank

    self.get_ifo() -> The ifo that the template bank is intended to be used
        for. If a bank (such as a pregenerated bank) is intended for *all* ifos
        then you should create multiple Tmpltbank classes, each with a different
        ifo.
        Will fail if the ifo has not yet been set.
    self.set_ifo(ifo) -> Sets the ifo used for this bank

    self.get_time() -> Returns a glue.ligolw segmentlist. This is the time for
        which the template bank should be valid. The bank may be generated for
        part of this time, or even a completely different time. This simply says
        that during the times given in the segmentlist, this template bank is
        the preferred one to use.
        Will fail if the time has not yet been set.
    self.set_time(segList) -> Sets the time for which the template bank is
        valid.

    self.get_job() -> Returns the condor job that will generate the template
        bank. If no job has been set, this will return None. For the case where
        the bank will not be generated from within the workflow, this should
        not be set.
    self.set_job(job) -> Sets the condor job that will run the template bank

    self.__init__() -> Initialize the class. Can take the following kwargs:
        bank=filePath -> Call self.set_bank(filePath), if not given will do
                         nothing
        ifo=ifo -> Call self.set_ifo(ifo), if not given will do nothing
        time=segList -> Call self.set_time(segList), if not given will do
                        nothing
        job=job -> Call self.set_job(job), if not given will do nothing
    '''
