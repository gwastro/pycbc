...
def f_FRD(m1, m2):
    ...
    return tmp / (2.0 * lal.PI * m_total * lal.MTSUN_SI)
...
def _get_imr_duration(m1, m2, s1z, s2z, f_low, approximant="SEOBNRv4"):
    ...
    m1, m2, s1z, s2z, f_low = float(m1), float(m2), float(s1z), float(s2z), float(f_low)
...
def get_inspiral_tf(
    ...
    if approximant in ["TaylorF2", "SPAtmplt"]:
        ...
        track_f = numpy.logspace(
            numpy.log10(f_low), numpy.log10(0.999 * f_high), n_points
        )
        track_t = numpy.array(
            [
...
def _energy_coeffs(m1, m2, chi1, chi2):
    ...
    energy6 = (
        ...
        - (155 * pow(eta, 2)) / 96.0
        - (35 * pow(eta, 3)) / 5184.0
        + eta * (59.80034722222222 - (205 * pow(lal.PI, 2)) / 96.0)
    )
...
def _dtdv_coeffs(m1, m2, chi1, chi2):
    ...
    dtdv3 = -4.0 * lal.PI + beta
    dtdv4 = (
        (3058673.0 + 5472432.0 * eta + 4353552.0 * eta * eta) / 1016064.0
        - sigma12
        - sigmaqm
    )
    dtdv5 = (1.0 / 672.0) * lal.PI * (-7729.0 + 1092.0 * eta) + (
    ...
    dtdv7 = (lal.PI / 1016064.0) * (
        -15419335.0 - 12718104.0 * eta + 4975824.0 * eta * eta
    )
...
def energy_coefficients(m1, m2, s1z=0, s2z=0, phase_order=-1, spin_order=-1):
    ...
    if phase_order >= 6:
        ecof[6] = (
            -675.0 / 64.0
            + (34445.0 / 576.0 - 205.0 / 96.0 * lal.PI * lal.PI) * eta
            - (155.0 / 96.0) * eta * eta
            - 35.0 / 5184.0 * eta * eta
        )
    # Spin terms
