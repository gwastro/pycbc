...
def filter_bin_lo_hi(values, lo, hi):
    ...
    if np.any(in_bin == 0):
        raise RuntimeError(
            "Edge case! Bin edges", lo, hi, "value(s)", values[in_bin == 0]
        )
    return in_bin == 1
    ...
    for t in trigs:
        if t >= np.max(bins):
            ...
            if counts[i] == 0:
                counts[i] = 1
            log_rhos.append(
                np.log(counts[i]) - np.log(bins[i + 1] - bins[i]) - np.log(N)
            )
...
def log_rho_fg_analytic(trigs, rhomin):
    ...
    return np.log(3.0) + 3.0 * np.log(rhomin) - 4 * np.log(trigs)
...
def alltimes(ifos, mincount=1):
    ...
    return it_chain.from_iterable(
        it_comb(ifos, r) for r in np.arange(mincount, len(ifos) + 1)
    )
...
class EventRate:
    ...
    def make_bins(self, maxval, choice="bg"):
        ...
        if linbw is not None:
            ...
            bins = np.logspace(
                np.log10(self.thr) - 0.0001, np.log10(maxval), n_bins + 1
            )
        else:
            raise RuntimeError(
                "Can't make bins without a %s bin width option!" % choice
            )
        if self.args.verbose:
    ...
    def moreifotimes(self, ctstring):
        ...
        allctime_moreifos = [
            ct for ct in self.allctimestring if len(ct) > len(ctstring)
        ]
        ...
        for st in self.starttimes:
            for ct in self.allctimestring:
                for cty in self.coinc_types:
                    ...
                    if self.args.verbose:
                        print(
                            "Found sg PDFs for %s coincs in %s time from %s"
                            % (cty, ct, st)
                        )
...
class BackgroundEventRate(EventRate):
    ...
    def add_background(self, full_file):
        ...
        with HFile(full_file, "r") as ff:
            ...
            for cty in self.coinc_types:
                ...
                self.bg_hist[(start, cty)] = np.histogram(
                    _bgstat[_ctype == cty], weights=_bgdec[_ctype == cty], bins=bins
                )
                # get expected number of bg events for this chunk and coinc type
                self.exp_bg[(start, cty)] = (
                    _bgdec[_ctype == cty].sum()
    ...
    def plot_bg(self):
        ...
        for chunk_type, hist in self.bg_hist.items():
            ...
            plt.savefig(
                self.args.plot_dir
                + "%s-bg_pdf-%s" % (chunk_type[1], chunk_type[0])
                + ".png"
            )
            plt.close()
...
class SignalEventRate(EventRate):
    ...
    def add_injections(self, inj_file, fg_file):
        ...
        with HFile(inj_file, "r") as jf:
            ...
            for ct in self.allctimestring:
                ...
                intime = self.in_coinc_time_excl(HFile(fg_file, "r"), ct, meantimes)
                ...
                for cty in self.coinc_types:
                    ...
                    if len(my_vals) > 0:
                        self.inj_vals[(ct, cty)] = np.append(
                            self.inj_vals[(ct, cty)], my_vals
                        )
                del intime, my_vals
    ...
    def plot_inj(self):
        ...
        for ct in self.allctimestring:
            for cty in self.coinc_types:
                ...
                plt.xlim(xmin=self.thr, xmax=2.0 * self.args.plot_max_stat)
                plt.ylim(ymin=0.7 * np.exp(logpdf.min()))
                plt.title(
                    r"%i injs plotted, \# of bins %i" % (len(samples), len(bins) - 1)
                )
                plt.xlabel("Ranking statistic")
    ...
    def eval_pdf(self, chunk, ctime, ctype, statvals):
        ...
        local_pdfs, _ = log_rho_fg(
            statvals, self.inj_vals[time_type], self.fg_bins[time_type]
        )
