...
def odds_summary(
    ...
    mchirps=None,
    name="events",
    plot_extensions=None,
):

    print("\nSummary of Top %i %s" % (ntop, name.title()))

    # do sort in reverse order
    statsort = numpy.argsort(1.0 / numpy.array(rankstats))
    ...
    for n, i in enumerate(topn):
        ...
        print("    IFAR     = %.2f" % ifar)
        print("    odds     = %g" % ((1.0 - p_b[i]) / p_b[i]))
        toppastro.append(1.0 - p_b[i])
        ...
        if hasattr(args, "json_min_ifar") and args.json_min_ifar is not None:
            for g, ifar, pt in zip(times, ifars, p_b):
                if ifar < args.json_min_ifar:
                    continue
                dump_json(g, 1.0 - pt, pt)
        else:
            for g, pa in zip(topgps, toppastro):
                dump_json(g, pa, 1.0 - pa)
...
def dist_summary(
    args,
    rv,
    plot_styles=("linear", "loglog", "semilogx"),
    plot_extensions=None,
    middle=None,
...
__all__ = ["dist_summary", "odds_summary", "plotdist"]
