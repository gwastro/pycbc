...
def save_bkg_falloff(fname_statmap, fname_bank, path, rhomin, lo_mchirp, hi_mchirp):
    ...
    with HFile(fname_bank, "r") as bulk:
        ...
        full_data = process_full_data(
            fname_statmap, rhomin, mass1_bank, mass2_bank, lo_mchirp, hi_mchirp
        )

    max_bg_stat = np.max(full_data["cstat_back_exc"])
    bg_bins = np.linspace(rhomin, max_bg_stat, 76)
    bg_counts = np.histogram(
        full_data["cstat_back_exc"], weights=full_data["dec_factors"], bins=bg_bins
    )[0]
...
def fgmc(log_fg_ratios, mu_log_vt, sigma_log_vt, Rf, maxfg):
    ...
    """
    Lb = np.random.uniform(0.0, maxfg, len(Rf))
    pquit = 0
...
def prob_lnm(m1, m2, s1z, s2z, **kwargs):
    ...
    **kwargs: string
    ...
    """
    min_mass = kwargs.get("min_mass", 5.0)
    max_mass = kwargs.get("max_mass", 95.0)
    max_mtotal = min_mass + max_mass
    m1, m2 = np.array(m1), np.array(m2)

    C_lnm = integrate.quad(
        lambda x: (log(max_mtotal - x) - log(min_mass)) / x, min_mass, max_mass
    )[0]
...
def prob_imf(m1, m2, s1z, s2z, **kwargs):
    ...
    **kwargs: string
    ...
    idx = np.where(m1 <= max_mtotal / 2.0)
    p_m1_m2[idx] = (1.0 / C_imf) * m1[idx] ** alpha / (m1[idx] - min_mass)
    idx = np.where(m1 > max_mtotal / 2.0)
    p_m1_m2[idx] = (1.0 / C_imf) * m1[idx] ** alpha / (max_mass - m1[idx])
    p_m1_m2[idx] = 0
...
def prob_flat(m1, m2, s1z, s2z, **kwargs):
    ...
    **kwargs: string
    ...
    """
    min_mass = kwargs.get("min_mass", 1.0)
    max_mass = kwargs.get("max_mass", 2.0)

    bound = np.sign(m1 - m2)
    bound += np.sign(max_mass - m1) * np.sign(m2 - min_mass)
    idx = np.where(bound != 2)

    p_m1_m2 = 2.0 / (max_mass - min_mass) ** 2
    p_m1_m2[idx] = 0
...
def draw_imf_samples(**kwargs):
    ...
    **kwargs: string
    ...
    k = nsamples * int(1.5 + log(1 + 100.0 / nsamples))
