...
class DictWithDefaultReturn(defaultdict):
    ...
    def __bool__(self):
        if self.items() and not all(entry is None for entry in self.values()):
            ...
            return True
        if self["RANDOM_STRING_314324"] is not None:
...
class MultiDetMultiColonOptionAction(MultiDetOptionAction):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        for value in values:
            if ":" not in value:
                err_msg += "Each argument must contain at least one ':' character"
                raise ValueError(err_msg)
        ...
        for value in values:
            ...
            if value.count(":") == 2:
                ...
                if param in detector_args[detector]:
                    err_msg += (
                        "Multiple values supplied for the same "
                        "parameter {} under detector {},\n"
                        "already have {}."
                    )
                    err_msg = err_msg.format(
                        param, detector, detector_args[detector][param]
                    )
                else:
                    detector_args[detector][param] = self.internal_type(val)
            elif value.count(":") == 1:
                param, val = value.split(":")
                for detector in namespace.instruments:
                    if detector not in detector_args:
                        detector_args[detector] = {param: self.internal_type(val)}
                    if param in detector_args[detector]:
                        err_msg += (
    ...
    for name in opt_list:
        ...
        if hasattr(opt, attr) and (getattr(opt, attr) is not None):
            if the_one is None:
                ...
                parser.error("%s and %s are mutually exculsive" % (the_one, name))

    if the_one is None:
        parser.error("you must supply one of the following %s" % (", ".join(opt_list)))
...
def angle_as_radians(s):
    ...
    try:
        value = float(s)
        warnings.warn(f"Angle units not specified for {value}, assuming radians")
        return value
    except:
        pass
