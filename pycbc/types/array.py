...
def check_same_len_precision(a, b):
    ...
    """
    if len(a) != len(b):
        msg = f"lengths do not match ({len(a)} vs {len(b)})"
        raise ValueError(msg)
    if a.precision != b.precision:
        msg = f"precisions do not match ({a.precision} vs {b.precision})"
        raise TypeError(msg)
...
class Array:
    ...
    def __init__(self, initial_array, dtype=None, copy=True):
        ...
        if copy:
            ...
            elif initial_array.dtype.kind == "c":
                dtype = complex128
            else:
                dtype = float64
