...
from pycuda.gpuarray import GPUArray, _get_common_dtype, empty
from pycuda.reduction import ReductionKernel
from pycuda.scan import InclusiveScanKernel
from pycuda.tools import (
...
class LowerLatencyReductionKernel(ReductionKernel):
    ...
    def __call__(self, *args, **kwargs):
        ...
        result, block_count, seq_count, grid_size, block_size = call_prepare(
            self, sz, args[0].allocator
        )
        ...
        while True:
            ...
            f(
                ...
                None,
                *(
                    [result.gpudata, result2.gpudata]
                    + s1_invocation_args
                    + [seq_count, sz]
                ),
                shared_size=self.shared_size,
...
def get_weighted_inner_kernel(dtype_x, dtype_y, dtype_w, dtype_out):
    ...
    return LowerLatencyReductionKernel(
        dtype_out,
        neutral="0",
...
if type(pycuda).__name__ not in ("MagicMock", "_MockModule"):
    ...
    amls = LowerLatencyReductionKernel(
        maxloc_dtype_single,
        neutral="maxloc_start()",
        reduce_expr="maxloc_red(a, b)",
        map_expr="maxloc_map(abs(x[i]), i)",
        arguments="float *x",
        preamble=maxloc_preamble_single,
    )

    amld = LowerLatencyReductionKernel(
        maxloc_dtype_double,
        neutral="maxloc_start()",
        reduce_expr="maxloc_red(a, b)",
        map_expr="maxloc_map(abs(x[i]), i)",
    ...
    amlsc = LowerLatencyReductionKernel(
        maxloc_dtype_single,
        neutral="maxloc_start()",
        reduce_expr="maxloc_red(a, b)",
        map_expr="maxloc_map(abs(x[i]), i)",
    ...
    amldc = LowerLatencyReductionKernel(
        maxloc_dtype_double,
        neutral="maxloc_start()",
        reduce_expr="maxloc_red(a, b)",
        map_expr="maxloc_map(abs(x[i]), i)",
    ...
    abs_max_loc_map = {
        "single": {"real": amls, "complex": amlsc},
        "double": {"real": amld, "complex": amldc},
    }
else:
    max_loc_map = {}
    abs_max_loc_map = {}
