...
#   General taper class
#
class TaperBoundsError(Exception):
    """This should be raised if applying a taper to a data set would result
...
class TimeDomainTaper(object):
    ...
    Available tapers are `lal` or any window recognized by
    ...
    initialization, and a time to use it when being applied (see `apply_taper`
    ...
    Tapers may be applied to unwhitened, whitened, or over-whitened data. If
    ...
    psds : (dict of) FrequencySeries, optional
        Required if `taper_whitened` is 1 or 2. Either a FrequencySeries or a
    ...
    ------
    TaperBoundsError
        Raised when `apply_taper` is called if the requested left (right) time
        occurs after (before) the end (start) of the data, such that the entire
        data segment would be zeroed.

    Example
    -------
    Create a taper window that uses a 5 second Hann window on the left and
    a 4 second Kaiser window with `beta = 8` on the right:

    >>> taperts = taper.TimeDomainTaper(left_taper='hann', right_taper=('kaiser', 8.), left_taper_duration=5., right_taper_duration=4.)

    Create a 16s time series of ones, and apply the above taper to it, starting
    at 2s and ending at 15s:

    >>> ts = types.TimeSeries(numpy.ones(16), delta_t=1.)
    >>> taperts(ts, left_time=2., right_time=15.).data
    ...
    def apply_taper(self, h, left_time=None, right_time=None, ifo=None,
        ...
        left_time : float
            ...
            after the end of the data, a TaperBoundsError is raised.
        right_time : float
            ...
            of the data, a TaperBoundsError is raised.
        ifo : None, optional
        ...
        if left_time is not None and left_time >= end_time:
            raise TaperBoundsError("start of left taper occurs after the end "
                                   "of the data")
        if right_time is not None and right_time <= start_time:
            raise TaperBoundsError("end of the right taper occurs before the "
                                   "start of the data")
