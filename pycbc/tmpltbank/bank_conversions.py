...
def get_bank_property(parameter, bank, template_ids):
    ...
    # Duration may be in the bank, but if not, we need to calculate
        ...
        if fullband_req:
            ...
            fullband_dur = pnutils.get_imr_duration(
                *[bank[k][:][template_ids] for k in imr_args + ["f_lower"]], **kwargs
            )
    ...
    # Basic conversions
    elif parameter in mass_conversions:
        values = mass_conversions[parameter](
            bank["mass1"][:][template_ids], bank["mass2"][:][template_ids]
        )

    elif parameter in spin_conversions:
        values = spin_conversions[parameter](
