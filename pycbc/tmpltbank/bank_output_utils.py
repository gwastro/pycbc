...
from pycbc.constants import GAMMA, MTSUN_SI, PI, TWOPI
...
def convert_to_sngl_inspiral_table(params, proc_id):
    ...
    for values in params:
        ...
        tmplt.mchirp, _ = pnutils.mass1_mass2_to_mchirp_eta(tmplt.mass1, tmplt.mass2)
...
def calculate_ethinca_metric_comps(
    metricParams, ethincaParams, mass1, mass2, spin1z=0.0, spin2z=0.0, full_ethinca=True
):
    r"""
    ...
    """
    if (float(spin1z) != 0.0 or float(spin2z) != 0.0) and full_ethinca:
        raise NotImplementedError(
            "Ethinca cannot at present be calculated for nonzero component spins!"
        )
    f0 = metricParams.f0
    if f0 != metricParams.fLow:
        raise ValueError(
            "If calculating ethinca the bank f0 value must be equal to f-low!"
        )
    if ethincaParams.fLow is not None and (ethincaParams.fLow != metricParams.fLow):
        raise NotImplementedError(
            "An ethinca metric f-low different from the"
    ...
    gammaVals[0] = 0.5 * two_pi_flower_sq * (Js[(1, 0)] - (Js[(4, 0)] * Js[(4, 0)]))
    ...
    Psi[0][3, 0] = 0.0
    Psi[0][4, 0] = (
        (-3058673 / 508032 + 5429 * eta / 504 + 617 * eta * eta / 24) * v0cube * v0
    )
    Psi[0][5, 1] = (-7729 * PI / 126) * v0cube * v0 * v0 / 3
    Psi[0][6, 0] = (
        (128 / 15)
        * (-3 * a0 - a1 + a2 + 3 * a3 + 107 * (1 + 3 * alog) / 14)
        * v0cube
        * v0cube
    )
    ...
    Psi[1][3, 0] = -3 / 2
    Psi[1][4, 0] = (
        (15293365 / 4064256 - 27145 * eta / 16128 - 3085 * eta * eta / 384) * v0 / PI
    )
    Psi[1][5, 1] = (193225 / 8064) * v0 * v0 / 3
    Psi[1][6, 0] = (
        (4 / PI)
        * (2 * a0 + a1 / 3 - 4 * a2 / 3 - 3 * a3 - 107 * (1 + 6 * alog) / 42)
        * v0cube
    )
    Psi[1][6, 1] = (-428 / PI / 7) * v0cube / 3
    Psi[1][7, 0] = (
        (77096675 / 1161216 + 378515 * eta / 24192 + 74045 * eta * eta / 8064)
        * v0cube
        * v0
    )
    ...
    for m in [0, 1]:
        for k in PNterms:
            gammaVals[1 + m] += (
                ...
                * two_pi_flower_sq
                * Psi[m][k]
                * (Js[(9 - k[0], k[1])] - Js[(12 - k[0], k[1])] * Js[(4, 0)])
            )
...
def output_sngl_inspiral_table(
    outputFile, tempBank, programName="", optDict=None, outdoc=None, **kwargs
    ...
    proc = create_process_table(
        outdoc, program_name=programName, detectors=ifos, options=optDict
    )
    ...
    if (
        "f_low_column" in optDict
        and "f_low" in optDict
        and optDict["f_low_column"] is not None
    ):
...
def output_bank_to_hdf(
    ...
    **kwargs,
    ...
    if optDict is not None:
        bank_dict["f_lower"] = numpy.ones_like(mass1) * optDict["f_low"]
        argument_string = [f"{k}:{v}" for k, v in optDict.items()]

    if optDict is not None and optDict["output_f_final"]:
        bank_dict["f_final"] = numpy.ones_like(mass1) * optDict["f_upper"]
...
def output_bank_to_file(outputFile, tempBank, **kwargs):
    if outputFile.endswith((".xml", ".xml.gz", ".xmlgz")):
        output_sngl_inspiral_table(outputFile, tempBank, **kwargs)
    elif outputFile.endswith((".h5", ".hdf", ".hdf5")):
        output_bank_to_hdf(outputFile, tempBank, **kwargs)
    else:
