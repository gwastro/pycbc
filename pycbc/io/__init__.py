...
def get_file(url, retry=5, backoff_cap=5, **args):
    ...
    Kept as it was on master, deliberately.  An earlier version of this branch
    deleted the mirror fall-back on the grounds that the caching proxy
    replaces it.  It does not: the mirror is what makes the *first* fetch work,
    and the cache is what avoids fetching at all, so they cover different
    failures.  With the mirror gone and a cold cache, the tutorial job spent
    seventy seconds timing out against gwosc.org over a JSON request that on
    master is answered from gwastro/pycbc_data.  It can come out once the cache
    is reliably warm, and not before.
