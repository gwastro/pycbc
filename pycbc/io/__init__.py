...
def get_file(url, retry=5, backoff_cap=5, **args):
    ...
    which is every caller: there is no CI-specific behaviour in this function
    any more.  It used to rewrite GWOSC and DCC requests to mirrored copies in
    gwastro/pycbc_data, but only when GITHUB_ACTIONS was set -- library code
    that behaved differently inside CI, with a list of mirror hosts and an
    md5-of-the-URL naming scheme to keep in step by hand.

    That job now belongs to the download cache, which is not specific to this
    project and needs no manifest.  The one thing the mirror did that a cache
    cannot is make the *first* fetch work, so a cold cache still depends on
    the origin answering once; the retry below is what covers a transient
    failure there.
