...
class UniformSky(angular.UniformSolidAngle):
    ...
    def to_uniform_patch(self, coverage):
        if coverage < 1:
            logging.warning(
                "Attempt to convert UniformSky to a uniform patch assumes 100% coverage"
            )
        return self
...
class UniformDiskSky:
    ...
    def __init__(self, **params):
        ...
        if radius < 0 or radius > 2 * numpy.pi:
            raise ValueError("Radius must be non-negative and smaller than 2π")
        # Prepare a rotation that puts the North Pole at the mean position
        self.rotation = Rotation.from_euler("yz", [numpy.pi / 2 - mean_dec, mean_ra])
    ...
    def rvs(self, size):
        ...
        np_dec = (
            angular.SinAngle(polar_bounds=(0, self.radius))
            .rvs(size=size)
...
class FisherSky:
    ...
    def to_uniform_patch(self, coverage):
        ...
        return UniformDiskSky(
            mean_ra=self.mean_ra, mean_dec=self.mean_dec, radius=radius
        )
...
class HealpixSky:
    ...
    def __init__(self, **params):
        ...
        if self.healpix_map.density():
            ...
            pix_areas = self.healpix_map.pixarea(numpy.arange(self.healpix_map.npix))
    ...
    def get_max_prob_point(self):
        coords = self.healpix_map.pix2ang(
            numpy.where(self.pix_probs == self.pix_probs.max())[0], lonlat=True
        )
    ...
    def pixel_corners(self, indices):
        ...
        """
        return numpy.array([self.healpix_map.boundaries(pi, step=1) for pi in indices])
    ...
    def rvs(self, size):
        ...
        # away.
        boundaries_phi = numpy.arctan2(boundaries_vec[:, 1, :], boundaries_vec[:, 0, :])
        del boundaries_vec
        ...
        boundaries_phi_min[straddler_mask] = boundaries_phi[straddler_mask, :].min(
            axis=1
        )
        boundaries_phi_max[straddler_mask] = boundaries_phi[straddler_mask, :].max(
            axis=1
        )
        del boundaries_phi
        ...
        while len(pix_indices) > 0:
            ...
            sampled_indices = self.healpix_map.ang2pix(random_thetas, random_phis)
            acceptance_mask = sampled_indices == pix_indices
            final_thetas = numpy.concatenate(
                (final_thetas, random_thetas[acceptance_mask])
            )
            final_phis = numpy.concatenate((final_phis, random_phis[acceptance_mask]))
...
__all__ = ["FisherSky", "HealpixSky", "UniformDiskSky", "UniformSky"]
