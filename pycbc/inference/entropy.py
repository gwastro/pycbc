...
def check_hist_params(samples, hist_min, hist_max, hist_bins):
    ...
    """
    hist_methods = ["auto", "fd", "doane", "scott", "stone", "rice", "sturges", "sqrt"]
    if not hist_bins:
        hist_bins = "fd"
    elif isinstance(hist_bins, str) and hist_bins not in hist_methods:
        raise ValueError(
            f"Method for calculating bins width must be one of {hist_methods}"
        )
...
def compute_pdf(samples, method, bins, hist_min, hist_max):
    ...
    if method == "kde":
        ...
        npts = max(10000, len(samples))
        draw = samples_kde.resample(npts)
        pdf = samples_kde.evaluate(draw)
    elif method == "hist":
        hist_range, hist_bins = check_hist_params(samples, hist_min, hist_max, bins)
        pdf, _ = numpy.histogram(
            samples, bins=hist_bins, range=hist_range, density=True
        )
    else:
...
def kl(
    ...
    if pdf1 and pdf2 and kde:
        raise ValueError(
            "KDE can only be used when at least one of pdf1 or pdf2 is False."
        )
...
def js(
    samples1, samples2, kde=False, bins=None, hist_min=None, hist_max=None, base=numpy.e
):
    """
    ...
    pdfs["M"] = (1.0 / 2) * (pdfs["P"] + pdfs["Q"])

    js_div = 0
    for pdf in (pdfs["P"], pdfs["Q"]):
        js_div += (1.0 / 2) * kl(pdf, pdfs["M"], pdf1=True, pdf2=True, base=base)
