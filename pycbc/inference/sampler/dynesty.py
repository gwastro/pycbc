...
class DynestySampler(BaseSampler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        log_likelihood_call, prior_call = setup_calls(
            model, loglikelihood_function=loglikelihood_function, copy_prior=True
        )
        # Set up the pool
        ...
        if self.nlive < 0:
            ...
            self.run_with_checkpoint = False
            logging.info(
                "Checkpointing not currently supported withDYNAMIC nested sampler"
            )
        else:
        ...
        if self.run_with_checkpoint is True:
            ...
            while diff_niter != 0:
                ...
                if delta_t >= self.checkpoint_time_interval:
                    logging.info(f"Checkpointing N={n_checkpointing}")
                    self.checkpoint()
                    n_checkpointing += 1
                    t0 = time.time()
                it = self._sampler.it
    ...
    def resume_from_checkpoint(self):
        try:
            ...
            logging.info("Found valid checkpoint file: %s", self.checkpoint_file)
...
def sample_rwalk_mod(args):
    ...
    try:
        # dynesty <= 1.1
        from dynesty.utils import reflect, unitcheck
        ...
        # dynest >= 1.2
        from dynesty.utils import apply_reflect as reflect
        from dynesty.utils import unitcheck

        (u, loglstar, axes, scale, prior_transform, loglikelihood, _, kwargs) = args
    ...
    while ii < nact * act:
        ...
        if accept + reject > maxmcmc:
            logging.warning(
                f"Hit maximum number of walks {maxmcmc} with accept={accept}, reject={reject}, "
                f"and nfail={nfail} try increasing maxmcmc"
            )
            break

    # If the act is finite, pick randomly from within the chain
    if numpy.isfinite(act) and int(0.5 * nact * act) < len(u_list):
        idx = numpy.random.randint(int(0.5 * nact * act), len(u_list))
        u = u_list[idx]
        v = v_list[idx]
        logl = logl_list[idx]
    else:
        logging.debug("Unable to find a new point using walk: returning a random point")
