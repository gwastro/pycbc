...
class NetBank(DummySampler):
    ...
    def run(self):
        ...
        node_loglrs = []
        for i in tqdm.tqdm(range(len(params[p]))):
            self.model.update(**{p: params[p][i] for p in self.model.variable_params}) 
        ...
        logging.info("Possible unique values", lengths[passed].sum())
        logging.info("Templates drawn from", len(numpy.unique(draw)))
        logging.info("Unique values first draw", len(upsamp))

        snrs2 = []
        for i, s in tqdm.tqdm(enumerate(upsamp), total=len(upsamp), position=0, leave=True):
            self.model.update(**{p: params[p][i] for p in self.model.variable_params}) 
            ll =  self.model.loglr
            snrs2.append((ll*2)**0.5)
            logw3[i] += ll

        snrs2 = numpy.array(snrs2)[expand]
        logw3 = logw3[expand] - numpy.array(lls)[draw]

        logw3 -= logsumexp(logw3)
        weight2 = numpy.exp(logw3)

        draw2 = choice(len(psamp), size=200000, replace=True, p=weight2)
        snrs2 = numpy.array(snrs2)[draw2]
        print("Unique values second draw", len(numpy.unique(snrs2)))
