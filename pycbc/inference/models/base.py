...
class BaseModel(metaclass=ABCMeta):
    ...
    def _logprior(self):
        ...
        # allow using constraint in the config which sets vector marginalized parameters
        if hasattr(self, 'marginalize_vector_params'):
            self.current_params_no_vec_margin = {
                k: v for k, v in self.current_params.items()
                if k not in self.marginalize_vector_params
            }
        logp = self.prior_distribution(**self.current_params_no_vec_margin) + logj
        if numpy.isnan(logp):
            logp = -numpy.inf
        return logp
