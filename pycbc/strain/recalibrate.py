...
class PhysicalModel:
    ...
    def __init__(
        ...
        init_detuning = self.freq**2 / (
            self.freq**2 - 1.0j * self.freq * self.fs0 * self.qinv0 + self.fs0**2
        )
    ...
    def update_c(self, fs=None, qinv=None, fc=None, kappa_c=1.0):
        ...
        """
        detuning_term = self.freq**2 / (
            self.freq**2 - 1.0j * self.freq * fs * qinv + fs**2
        )
    ...
    def map_to_adjust(self, strain, **params):
        ...
        strain_adjusted = self.adjust_strain(
            strain,
            delta_fs=calib_args[0],
            delta_fc=calib_args[1],
            delta_qinv=calib_args[2],
...
def read_calibration_envelop_file(
    calibration_file, correction_type, minimum_frequency, maximum_frequency, n_nodes
):
    """
    ...
    log_nodes = np.linspace(
        np.log(minimum_frequency), np.log(maximum_frequency), n_nodes
    )
    ...
    if calibration_data[0][-1] < maximum_frequency:
        if maximum_frequency - calibration_data[0][-1] < 0.5:
            ...
            raise ValueError(
                "Maximum frequency (=%d) for tc=%s is more than"
                "maximum frequency in calibration envelop (=%.3g)"
                % (maximum_frequency, gps_time, calibration_data[0][-1])
            )
    else:
        pass
    ...
    if calibration_data[0][0] > minimum_frequency:
        raise ValueError(
            "Minimum frequency (=%d) for tc=%s is less than"
            "minimum frequency in calibration envelop (=%.3g)"
            % (minimum_frequency, gps_time, calibration_data[0][0])
        )

    amplitude_median_nodes = InterpolatedUnivariateSpline(
        log_frequency_array, amplitude_median
    )(log_nodes)
    amplitude_sigma_nodes = InterpolatedUnivariateSpline(
        log_frequency_array, amplitude_sigma
    )(log_nodes)
    phase_median_nodes = InterpolatedUnivariateSpline(
        log_frequency_array, phase_median
    )(log_nodes)
    phase_sigma_nodes = InterpolatedUnivariateSpline(log_frequency_array, phase_sigma)(
        log_nodes
    )
...
def get_calibration_files(ifos, gps_time, calibration_file_path):
    ...
    for ifo in ifos:
        if RUN_NAME == "O4a":
            all_calibration_files = glob.glob(
                "%s/%s_%s/*.txt" % (calibration_file_path, ifo, RUN_NAME)
            )
        else:
            all_calibration_files = glob.glob(
                "%s/%s/*FinalResults.txt" % (calibration_file_path, ifo)
            )
        if ifo != "V1":
            ...
            if RUN_NAME == "O4a":
                list_gpstimes = np.array(
                    [
                        int(item.split("_")[-1].split(".")[0])
                        for item in all_calibration_files
                    ]
                )
            else:
                list_gpstimes = np.array(
                    [int(item.split("_")[-4]) for item in all_calibration_files]
                )
            dt_list = abs(list_gpstimes - gps_time)
            ifo_calibration_file = all_calibration_files[dt_list.argmin()]
        elif RUN_NAME == "O2":
            ifo_calibration_file = (
                "%s/calibration_envelops/V1/V_calibrationUncertaintyEnvelope_magnitude5p1percent_phase40mraddeg20microsecond.txt"
                % os.getcwd()
            )
        elif RUN_NAME == "O3a":
            ifo_calibration_file = (
                "%s/calibration_envelops/V1/V_O3a_calibrationUncertaintyEnvelope_magnitude5percent_phase35milliradians10microseconds.txt"
                % os.getcwd()
            )
        elif RUN_NAME == "O3b":
            ifo_calibration_file = (
                "%s/calibration_envelops/V1/V_O3b_calibrationUncertaintyEnvelope_magnitude5percent_phase35milliradians10microseconds.txt"
                % os.getcwd()
            )
        else:
        ...
        if RUN_NAME == "O4a":
            dict_calibration_file[ifo] = "%s/%s_%s/%s" % (
                calibration_file_path,
                ifo,
                RUN_NAME,
                ifo_calibration_file.split("/")[-1],
            )
        else:
            dict_calibration_file[ifo] = "%s/%s/%s" % (
                calibration_file_path,
                ifo,
                ifo_calibration_file.split("/")[-1],
            )
