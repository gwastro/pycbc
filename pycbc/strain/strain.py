...
def from_cli(opt, dyn_range_fac=1, precision="single", inj_filter_rejector=None):
    ...
    if precision == "single":
        ...
        raise ValueError(f"Unrecognized precision {precision}")
...
def from_cli_multi_ifos(opt, ifos, inj_filter_rejector_dict=None, **kwargs):
    ...
    if inj_filter_rejector_dict is None:
        inj_filter_rejector_dict = dict.fromkeys(ifos)
    for ifo in ifos:
...
class StrainBuffer(pycbc.frame.DataBuffer):
    def __init__(
        ...
        self.trim_padding = min(self.trim_padding, self.total_corruption)
    ...
    def advance(self, blocksize, timeout=10):
        ...
        """
        ts = super().attempt_advance(blocksize, timeout=timeout)
