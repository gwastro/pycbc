...
class PhaseTDStatistic(QuadratureSumStatistic):
    ...
    def get_hist(self, ifos=None):
        ...
        for ifo in self.hist_ifos:
            ...
            # renormalise to PDF
            self.weights[ifo] = weights[ifo] / (
                weights[ifo].sum() * bin_volume
            )
    ...
    def logsignalrate(self, stats, shift, to_shift):
        ...
        ps = {
            ifo: numpy.array(stats[ifo]["coa_phase"], ndmin=1)
            for ifo in self.ifos
        }
        ts = {
            ifo: numpy.array(stats[ifo]["end_time"], ndmin=1)
            for ifo in self.ifos
        }
        ss = {
            ifo: numpy.array(stats[ifo]["snr"], ndmin=1) for ifo in self.ifos
        }
        sigs = {
            ifo: numpy.array(stats[ifo]["sigmasq"], ndmin=1)
            for ifo in self.ifos
        }
