...
def insert_bank_bins_option_group(parser):
    ...
    """
    bins_group = parser.add_argument_group("Options for selecting templates in bins.")
    ...
    bins_group.add_argument(
        "--bank-file", default=None, help="HDF format template bank file."
    )
    bins_group.add_argument(
        "--f-lower", default=None, help="Low frequency cutoff in Hz."
    )
    return bins_group
...
def get_param(par, args, m1, m2, s1z, s2z):
    ...
    if par == "mchirp":
        ...
        parvals = pnutils.get_imr_duration(
            m1, m2, s1z, s2z, args.f_lower, args.approximant
        )
...
def get_found_param(injfile, bankfile, trigfile, param, ifo, args=None):
    ...
    if trigfile is not None:
        try:  # old 2-ifo behaviour
            ...
            ifolabel = [
                name
                for name, val in injfile.attrs.items()
                if "detector" in name and val == ifo
            ][0]
    ...
    if trigfile is not None and param in trigfile[ifo].keys():
        return trigfile[ifo][param][:][foundtrg], found_in_ifo
    assert bankfile
    b = bankfile
    return get_param(
        param, args, b["mass1"][:], b["mass2"][:], b["spin1z"][:], b["spin2z"][:]
    )[foundtmp], found_in_ifo
