...
from pycbc import DYN_RANGE_FAC, waveform
...
def compute_network_snr_core(
    ...
    if not hasattr(htilde, "params"):
        htilde.params = dict(mass1=mass1, mass2=mass2, spin1z=v[2], spin2z=v[3])
    if not hasattr(htilde, "end_idx"):
        htilde.end_idx = int(f_end / htilde.delta_f)
    htilde.approximant = approximant
    htilde.sigmasq = types.MethodType(pycbc.waveform.bank.sigma_cached, htilde)
    ...
    for ifo in ifos:
        ...
        snr, _, norm = matched_filter_core(htilde, data[ifo], h_norm=sigmasq)
        duration = 0.095
        ...
        network_snrsq += max(abs(snr_series._data)) ** 2.0
...
def optimize_shgo(bounds, cli_args, extra_args, initial_point):  # pylint: disable=unused-argument
    bounds = [bounds["mchirp"], bounds["eta"], bounds["spin1z"], bounds["spin2z"]]
    results = shgo(
        compute_minus_network_snr,
        bounds=bounds,
        args=extra_args,
...
def optimize_pso(bounds, cli_args, extra_args, initial_point):
    ...
    # Initialize the population with random values within specified bounds
    population = numpy.random.uniform(
        min_bounds, max_bounds, size=(int(cli_args.snr_opt_pso_particles), len(bounds))
    )
...
option_dict = {
    "differential_evolution": {
        ...
        "popsize": ("A multiplier for setting the total population size.", 100),
    },
    "shgo": {
        "samples": (
            "Number of sampling points used in the construction of "
    ...
    "pso": {
        "iters": ("Number of iterations used in the particle swarm optimization.", 5),
...
def insert_snr_optimizer_options(parser):
    ...
    opt_opt_group.add_argument(
        ...
        "or differential_evolution. Set to "
        "random"
        " for a random seed",
    )
...
def check_snr_optimizer_options(args, parser):
    ...
    for k in options:
        ...
        if any(options[k]):
            parser.error(
                "Argument has been supplied which is not suitable "
