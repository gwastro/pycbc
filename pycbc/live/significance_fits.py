...
def add_live_significance_trigger_pruning_options(parser):
    ...
    pruning_group.add_argument(
        "--prune-loudest",
        type=int,
        help="Maximum number of loudest trigger clusters to remove from each bin.",
    )
    pruning_group.add_argument(
        "--prune-window",
        type=float,
    ...
    pruning_group.add_argument(
        ...
        type=float,
        help="Minimum statistic value to consider a trigger for pruning.",
    )
...
def verify_live_significance_trigger_pruning_options(args, parser):
    ...
    prune_options = [args.prune_loudest, args.prune_window, args.prune_stat_threshold]

    if any(prune_options) and not all(prune_options):
        parser.error(
            "Require all or none of --prune-loudest, "
...
def add_live_significance_duration_bin_options(parser):
    ...
    durbin_group.add_argument(
        "--duration-from-bank",
        help="Path to the template bank file to get max/min durations from.",
    )
    durbin_group.add_argument(
        "--num-duration-bins",
        type=int,
        help="How many template duration bins to split the bank into before fitting.",
    )
    durbin_group.add_argument(
...
def verify_live_significance_duration_bin_options(args, parser):
    ...
    if args.duration_bin_edges:
        if (
            args.duration_bin_start
            or args.duration_bin_end
            or args.duration_from_bank
            or args.num_duration_bins
        ):
            parser.error(
                "Cannot use --duration-bin-edges with "
                "--duration-bin-start, --duration-bin-end, "
        ...
        if not args.num_duration_bins:
            parser.error(
                "--num-duration-bins must be set if not using --duration-bin-edges."
            )
        if not (
            (args.duration_bin_start and args.duration_bin_end)
            or args.duration_from_bank
        ):
            parser.error(
    ...
    if args.duration_bin_end and args.duration_bin_end <= args.duration_bin_start:
        parser.error(
            "--duration-bin-end must be greater than "
...
def duration_bins_from_cli(args):
    ...
    if args.duration_bin_spacing == "log":
        return numpy.logspace(
            numpy.log10(min_dur), numpy.log10(max_dur), args.num_duration_bins + 1
        )
    if args.duration_bin_spacing == "linear":
        return numpy.linspace(min_dur, max_dur, args.num_duration_bins + 1)
