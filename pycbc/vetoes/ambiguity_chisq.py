...
def segment_snrs(filters, stilde, psd, flow, fhigh):
    ...
    for template in filters:
        snr = pf.matched_filter(
            template, stilde, None, flow, fhigh, pf.sigmasq(template, psd, flow, fhigh)
        )
        snrs.append(snr)
    return snrs
...
def get_cov_matrix(snr, cov_gg, cov_gh):
    ...
    for i, j in itertools.combinations_with_replacement(range(len(cov_gh)), 2):
        cov[i, j] = cov_gg[i, j] - (
            snr.real * cov_gh[i].real + snr.imag * cov_gh[i].imag
        ) * (snr.real * cov_gh[j].real + snr.imag * cov_gh[j].imag)
        cov[j, i] = cov[i, j]
    return cov
...
def get_vector(snr, snr_id, seg_snrs, cov_gh):
    ...
    for i in range(len(cov_gh)):
        vec[i] = (
            seg_snrs[i][snr_id].real
            - snr.real * cov_gh[i].real
            - snr.imag * cov_gh[i].imag
        )
    return vec
...
def get_chisq(vec, eig, rot_mat, threshold=0.05):
    ...
    logging.info(
        "Eigenvalues min, max, ratio: {}, {}, {}".format(
            eig[rel_eig][0], eig[rel_eig][-1], eig[rel_eig][-1] / eig[rel_eig][0]
        )
    )
...
def inner(
    ...
    if _snr is None or _snr.dtype != htilde.dtype or len(_snr) != N:
        ...
        snr, corr, snr_norm = pf.matched_filter_core(
            htilde,
            stilde,
            psd,
...
class SingleDetAmbiguityChisq(object):
    ...
    def __init__(
        ...
        if status:
            ...
            if ("mchirp" not in bank.table.fieldnames) or (
                ...
                mchirp, eta = pnu.mass1_mass2_to_mchirp_eta(
                    bank.table["mass1"], bank.table["mass2"]
                )
            ...
            if use_full_bank:
                ...
                if per_template:
                    ...
                    self.choose_filters = FiltersForTemplate(
                        self.bank,
                        mc_bound=mc_bound,
                        eta_bound=eta_bound,
                        chie_bound=chie_bound,
                        min_filters=min_filters,
                        max_filters=max_filters,
                    )
                    self.get_filters = self.choose_filters.get_relevant_filters
                else:
                    self.choose_filters = FiltersRegions(
                        self.bank,
                        mc_bound=mc_bound,
                        eta_bound=eta_bound,
                        chie_bound=chie_bound,
    ...
    def insert_option_group(parser):
        ...
        group.add_argument(
            "--ambi-condition-threshold",
            type=float,
            default=0.05,
            help="Minimum SNR threshold to use ambi chisq",
        )
        group.add_argument(
            "--ambi-veto-bank-file", type=str, help="bank file for ambiguity chisq"
        )
        group.add_argument(
            "--ambi-veto-use-full-bank", action="store_true", default=False
        )
        group.add_argument(
            "--ambi-min-filters",
            type=int,
        ...
        group.add_argument(
            "--ambi-mc-bins", nargs="*", type=float, help="chirp mass bin boundaries"
        )
        group.add_argument(
            "--ambi-eta-bins",
            nargs="*",
            type=float,
            help="symmetric mass ratio bin boundaries: eta range [0.25, 0)",
        )
        group.add_argument(
            "--ambi-chi-eff-bins",
            nargs="*",
    ...
    def from_cli(cls, args, bank):  # , match_class):
        ...
        return cls(
            args.ambi_status,
            bank,
            args.ambi_snr_threshold,
            flow,
            fmax,
            mc_bound,
            eta_bound,
            chie_bound,
            args.ambi_min_filters,
            args.ambi_max_filters,
    ...
    def get_full_bank(self, htilde):
        mchirp, eta = pnu.mass1_mass2_to_mchirp_eta(
            htilde.params.mass1, htilde.params.mass2
        )
        chie = chi_eff(
            htilde.params.mass1,
            htilde.params.mass2,
        ...
        if self.do:
            ...
            if len(filters) > self.min_filters:
                ...
                cond_thre = (
                    ...
                    else self.condition_threshold
                )
                logging.info("Computing ambiguity chi-squares ...")
                chisq, dof = compute_chisq(
                    snrs[rel_ids], snr_ids[rel_ids], seg_snrs, cov_gg, cov_gh, cond_thre
                )
...
class FiltersRegions(object):
    def __init__(
        ...
        self.ranges = {
            ...
            if mc_bound
                ...
                [bank.table["mchirp"].min() * 0.95, bank.table["mchirp"].max() * 1.05]
            ),
            "eta": np.array(eta_bound),
            "chi_eff": np.array(chie_bound),
        }  ## make sure to have min and max of the mf banks covered
    ...
    def idx_from_ranges(self, htilde):
        ...
        idx = (
            np.abs(self.bank.table["mchirp"] - h_params["mchirp"]) > 1e-5
        )  # Just to make sure we are not having the trigger template
        logging.info("not me left over are {}".format(idx.sum()))
        for k, v in self.ranges.items():
            bin = np.searchsorted(v, h_params[k])
            if bin < 1 or bin >= len(
                v
            ):  # If out of boundaries given, we are not calculating anything
                idx *= np.zeros_like(idx)
                logging.info(
                    "after {}, out of the bound: left over are {}".format(k, idx.sum())
                )
                ...
                idx = (
                    idx
                    * (self.bank.table[k] > v[bin - 1])
                    * (self.bank.table[k] < v[bin])
                )
    ...
    def get_relevant_filters(self, htilde):
        ...
        if key not in self._cache_relevant_filters:
            ...
            if idx.sum() > self.max_filters:
                ...
                h_params = {"mchirp": mchirp, "eta": eta, "chi_eff": chie}
                filter_list.table = filter_list.table[
                    np.argsort(
                        np.abs(filter_list.table["mchirp"] - h_params["mchirp"])
                    )[: self.max_filters]
                ]
            self._cache_relevant_filters[key] = list(
                filter_list
            )  # list of filters to be used in chisq
