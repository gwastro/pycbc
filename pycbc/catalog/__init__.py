...
def find_event_in_catalog(name, source=None):
    ...
    """
    from .catalog import get_source, list_catalogs
...
class Merger:
    ...
    def median1d(self, name, return_errors=False):
        ...
        try:
            ...
            print(e)
            raise RuntimeError(f"Cannot get parameter {name}")
    ...
    def strain(self, ifo, duration=32, sample_rate=4096):
        ...
        for fdict in self.data["strain"]:
            ...
            raise ValueError(
                "no strain data is available as requested for " + self.common_name
            )

        ver = url.split("/")[-1].split("-")[1].split("_")[-1]
        sampling_map = {4096: "4KHZ", 16384: "16KHZ"}
        channel = f"{ifo}:GWOSC-{sampling_map[sample_rate]}_{ver}_STRAIN"
...
class Catalog:
    ...
    def __getitem__(self, key):
        try:
            ...
            for m in self.mergers:
                ...
                raise ValueError(f"Did not find merger matching name: {key}")
    ...
    def median1d(self, param, return_errors=False):
        ...
        """
        v = [
            self.mergers[m].median1d(param, return_errors=return_errors)
            for m in self.mergers
        ]
        if return_errors:
