...
def insert_args(parser):
    mchirp_group = parser.add_argument_group(
        "Arguments for estimating the "
        "source probabilities of a "
        "candidate event using the snr, "
        "mchirp, and effective distance."
    )
    mchirp_group.add_argument(
        "--src-class-mass-limits",
        type=float,
        nargs=3,
    ...
    mchirp_group.add_argument(
        "--src-class-lum-distance-to-delta",
        type=float,
        nargs=2,
        metavar=("b0", "b1"),
        required=True,
...
def redshift_estimation(distance, distance_std, lal_cosmology):
    ...
    z_est_min = _redshift((distance - distance_std), cosmology=cosmology)
...
def src_mass_from_z_det_mass(z, del_z, mdet, del_mdet):
    ...
    """
    msrc = mdet / (1.0 + z)
    del_msrc = msrc * ((del_mdet / mdet) ** 2.0 + (del_z / (1.0 + z)) ** 2.0) ** 0.5
...
def get_area(trig_mc, lim_h1, lim_h2, lim_v1, lim_v2):
    ...
    mi_max = (2.0**0.2) * mc_max
    mi_min = (2.0**0.2) * mc_min
...
def calc_areas(trig_mc_det, mass_limits, mass_bdary, z, mass_gap, mass_gap_separate):
    ...
    classifying diagram.
    """
    trig_mc = src_mass_from_z_det_mass(
        z["central"], z["delta"], trig_mc_det["central"], trig_mc_det["delta"]
    )
    ...
    if mass_gap:
        ...
        return {"BNS": abns, "NSBH": ansbh, "BBH": abbh, "Mass Gap": agns + agg + abhg}
    return {"BNS": abns, "NSBH": ansbh, "BBH": abbh}
...
def calc_probabilities(mchirp, snr, eff_distance, src_args):
    ...
    dist_std_estimation = dist_estimation * math.exp(coeff["b0"]) * snr ** coeff["b1"]
    z = redshift_estimation(
        dist_estimation, dist_std_estimation, src_args["lal_cosmology"]
    )
    ...
    if trig_mc_det["central"] > mc_max * (1 + z["central"]):
        if mass_gap:
            if mass_gap_separate:
                ...
                probabilities = {"BNS": 0.0, "NSBH": 0.0, "BBH": 1.0, "Mass Gap": 0.0}
        else:
        ...
        if mass_gap:
            if mass_gap_separate:
                ...
                probabilities = {"BNS": 1.0, "NSBH": 0.0, "BBH": 0.0, "Mass Gap": 0.0}
        else:
            probabilities = {"BNS": 1.0, "NSBH": 0.0, "BBH": 0.0}

    else:
        areas = calc_areas(
            trig_mc_det, mass_limits, mass_bdary, z, mass_gap, mass_gap_separate
        )
