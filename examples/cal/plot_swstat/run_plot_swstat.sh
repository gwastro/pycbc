...
GPS_END_TIME=$((${GPS_START_TIME} + 10))
IFO=L
FRAME_TYPE=L1_R
...
CHANNEL_NAME=L1:OAF-CAL_SUM_DARM_ERR_SWSTAT
...
echo ./pycbc_plot_swstat --gps-start-time ${GPS_START_TIME} --gps-end-time ${GPS_END_TIME} --frame-cache ${FRAME_CACHE} --filter-file ${FILTER_FILE} --channel-name ${CHANNEL_NAME}
