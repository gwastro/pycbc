...
datafind_files, scienceSegs = ahope.setup_datafind_workflow(workflow, scienceSegs,
                       dfDir, checkSegmentGaps='update_times', checkFramesExist='warn')

# Template bank stuff
bank_files = ahope.setup_tmpltbank_workflow(workflow, scienceSegs, datafind_files, 'tmpltbank')
splitbank_files = ahope.setup_splittable_workflow(workflow, bank_files, 'tmpltbank')

# Do matched-filtering and injections 
insps = ahope.setup_matchedfltr_workflow(workflow, scienceSegs, datafind_files, 
                                           splitbank_files, 'inspiral')
insps_set, inj_tags = ahope.setup_injection_workflow(workflow, scienceSegs, datafind_files, 
                                           splitbank_files, start_time, 
                                           end_time, 'inspiral')

all_insp = [insps] + insps_set
all_tags = [""] + inj_tags

coinc_sets = []
for insp, tag in zip(all_insp, all_tags):
    if tag == "":
        tag = []
    else:
        tag = [tag]
    coincs = ahope.setup_coincidence_workflow(workflow, scienceSegs, segsDict, 
                                              insp, 'coincidence', tags=tag,
                                              maxVetoCat = 1)
    coinc_sets.append(coincs)
