...
"""
Programe for running multi-detector pycbc analysis through coincidence and then
...
workflow = _workflow.Workflow(args, 'weekly_ahope')
...
splitbank_files = _workflow.setup_splittable_workflow(workflow, bank_files, dfDir) 
...
for category in range(1, 6):
    ...
    workflowVetoFile = segsFileList.find_output_with_tag(vetoTag)
    assert len(workflowVetoFile) == 1
    workflowVetoFile = workflowVetoFile[0]
    workflowVetoPath = workflowVetoFile.storage_path
    ...
    shutil.copyfile(workflowVetoPath, pipedownVetoPath)
...
_workflow.make_external_call(pipeCommand, out_dir=pipedownDir + "/logs", 
