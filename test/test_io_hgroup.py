    ...
    def test_create_dataset_has_checksum(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with HFile(p, "w") as f:
                ...
                f.create_dataset(
                    "shape_based_with_dtype", shape=(10,), dtype="f8"
                )
                ...
                f.create_dataset(
                    "data_obj_list", data=np.array(str_list, dtype=object)
                )
                should_not_be_checksummed.append("data_obj_list")

                # Make a datasets from scalars:
                f.create_dataset("data_scalar_with_dtype", data=0, dtype="f8")
    ...
    def test_getitem_returns_hgroup_and_root_parent_behaviour(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with HFile(p, "w") as f:
                ...
                root = f["/"]
                self.assertIsInstance(
                    root.parent, HGroup, "Expected root.parent to be an HGroup"
                )
                self.assertEqual(
                    root.parent.id,
                    root.id,
    ...
    def test_getitem_wraps_group_and_leaves_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with HFile(p, "r") as f:
                ...
                self.assertIsInstance(
                    got_grp, HGroup, f"Expected a HGroup, got {type(got_grp)}"
                )
    ...
    def test_dataset_parity_h5py_vs_hfile(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with h5py.File(p_hfile, "r") as fh, h5py.File(p_h5py, "r") as fp:
                ...
                self.assertEqual(
                    d_hfile.shape, d_h5py.shape, "Shapes should be identical"
                )
                self.assertEqual(
                    str(d_hfile.dtype),
                    str(d_h5py.dtype),
