...
parser.add_argument(
    "--output-file", type=str, required=True, help="Full path to output file"
)
parser.add_argument(
    "--loudest-event-number",
    type=int,
    required=True,
    default=1,
...
for era in eras:
    ...
    for file_name in file_list:
        ...
        for row in snglburst_table:
            if (
                row.snr > args.omicron_snr_thresh
...
plt.title(
    ...
    + " - "
    + format(m2, ".2f")
    + " solar masses at GPS time "
    + format(cbc_end_time, ".2f"),
    fontsize=12,
)
plt.hold(True)
plt.plot(times, freq)
